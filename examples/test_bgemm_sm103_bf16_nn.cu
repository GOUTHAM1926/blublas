// test_bgemm_sm103_bf16_nn.cu
// Benchmark and accuracy test: BluBridge SM103a BF16 batched NN GEMM vs cuBLAS.
// Target: NVIDIA B300 (SM 10.3a) — tcgen05.mma.cta_group::1.kind::f16
//
// Usage:
//   ./build/test_bgemm_sm103_bf16_nn
//
    ...
    cudaStream_t stream)
{
    ...
    const long long elB = (long long)batchCount * K * N; // NN layout: B is K x N
    ...
    for (int w = 0; w < 5; ++w)
        mycublasBgemmSM103_bf16_nn_128x128x64(
...
int main()
{
    ...
    {
        printf("[SKIP] SM103 BF16 NN kernel requires SM 10.3 (B300/sm_103a).\n"
