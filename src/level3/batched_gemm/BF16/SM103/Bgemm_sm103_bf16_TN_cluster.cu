...
//
// Each cluster: CTA0 → M cols [block_row0*BM..(block_row0+1)*BM), N cols [block_col*BN..+BN/2)
//               CTA1 → M cols [block_row1*BM..(block_row1+1)*BM), N cols [block_col*BN+BN/2..+BN)
...
static inline void init_4d_tma_A_TN(
...
static inline void init_4d_tma_B_TN(
    CUtensorMap* tmap, const __nv_bfloat16* ptr,
    ...
    uint64_t batchCount, uint64_t batchStride, CUtensorMapSwizzle sw)
{
    ...
    uint64_t globalStrides[rank-1]= {(uint64_t)N*sizeof(__nv_bfloat16), 128ULL,
                                      batchStride*sizeof(__nv_bfloat16)};
    uint32_t boxDim[rank]         = {64U, (uint32_t)BK, (uint32_t)(BN/CTA_GROUP_SIZE/64), 1U};
    uint32_t elementStrides[rank] = {1U,1U,1U,1U};
    check_cu(cuTensorMapEncodeTiled(tmap, CU_TENSOR_MAP_DATA_TYPE_BFLOAT16, rank, (void*)ptr,
        globalDim, globalStrides, boxDim, elementStrides,
        CU_TENSOR_MAP_INTERLEAVE_NONE, sw,
...
void bgemm_sm103_bf16_tn_cluster_kernel(
    ...
    int grid_n)
{
    ...
    constexpr int b_tile_bytes = BK * (BN / CTA_GROUP_SIZE) * (int)sizeof(__nv_bfloat16);
    ...
    if (warp_id == producer_warp && elect_sync())
    {
        ...
        while (true)
        {
            ...
            for (int k = 0; k < k_iters; ++k)
            {
                ...
                tma_4d_gmem2smem<CTA_GROUP_SIZE>(A_smem, &A_tmap, 0, k*BK, A_col, cur_batch, mbar);
                // B TN: y=k*BK (K row), z=B_col (N column group)
                tma_4d_gmem2smem<CTA_GROUP_SIZE>(B_smem, &B_tmap, 0, k*BK, B_col, cur_batch, mbar);
...
static void launch_bgemm_tn_cluster(
    ...
    int M, int N, int K, int batchCount, float alpha, float beta, cudaStream_t stream)
{
    ...
    if (s_arch_ok < 0) {
        ...
        if (!s_arch_ok)
            fprintf(stderr, "[BluBridge SM103 TN Cluster] WARNING: device '%s' SM %d.%d, requires SM 10.3+\n",
    ...
    constexpr int b_tile    = BK * (BN / CTA_GROUP_SIZE) * (int)sizeof(__nv_bfloat16);
    ...
    init_4d_tma_A_TN(&A_tmap, A, M, K, BM, BK,
                     (uint64_t)batchCount, (uint64_t)K*M, CU_TENSOR_MAP_SWIZZLE_128B);
    init_4d_tma_B_TN(&B_tmap, B, N, K, BN, BK,
                     (uint64_t)batchCount, (uint64_t)K*N, CU_TENSOR_MAP_SWIZZLE_128B);
...
static void dispatch_bgemm_tn_cluster(
    const __nv_bfloat16* A, const __nv_bfloat16* B, __nv_bfloat16* C,
    int M, int N, int K, int batchCount, float alpha, float beta, cudaStream_t stream)
{
    launch_bgemm_tn_cluster<128, 128, 64>(A, B, C, M, N, K, batchCount, alpha, beta, stream);
}
    ...
    float beta, __nv_bfloat16* C, int batchCount)
{
    ...
    if (M % (CTA_GROUP_SIZE * 256) == 0)
        launch_bgemm_tn_cluster<256, 256, 64>(A, B, C, M, N, K, batchCount, alpha, beta, stream);
    else
        dispatch_bgemm_tn_cluster(A, B, C, M, N, K, batchCount, alpha, beta, stream);
}
