// Bgemm_sm103_bf16_NN.cu
// Blackwell Ultra (SM103a / B300) BF16 Batched GEMM — NN layout
...
static inline void init_4d_tma_map_sm103_A(
    CUtensorMap* tmap,
    const __nv_bfloat16* ptr,
    int K, int BK,
    uint64_t global_height,   // M for A
    uint32_t box_height,      // BM for A
...
// 4D TMA for B in NN layout.
// B[batch, K, N] row-major -> B is K rows, N cols.
static inline void init_4d_tma_map_sm103_B_NN(
    CUtensorMap* tmap,
    const __nv_bfloat16* ptr,
    int N, int BN,
    int K, int BK,
...
{
    constexpr uint32_t rank = 4;
    // B is [batch, K, N] row-major.
    // Inner dim = 64 elements (128 bytes).
    // Height = K. Stride is N * sizeof(bf16).
    // Depth = N / 64. Stride is 128 bytes.
    uint64_t globalDim[rank]      = {64ULL, (uint64_t)K,
                                      (uint64_t)(N / 64), batch_count};
    uint64_t globalStrides[rank-1] = {
        (uint64_t)N * sizeof(__nv_bfloat16),
        128ULL,
        batch_stride_elems * sizeof(__nv_bfloat16)
    };
    uint32_t boxDim[rank]         = {64U, (uint32_t)BK, (uint32_t)(BN / 64), 1U};
    uint32_t elementStrides[rank] = {1U, 1U, 1U, 1U};
...
__global__
void bgemm_sm103_bf16_nn_kernel(
    ...
    const __grid_constant__ CUtensorMap C_tmap)
{
    ...
    constexpr uint32_t i_desc =
        ...
        | (1U <<  7U)
        | (1U << 10U)
        | (1U << 16U)  // trans_b = 1 (NN layout transposed in tensor core)
        | ((uint32_t)BN  >> 3U << 17U)
        | ((uint32_t)BM  >> 4U << 24U);
    ...
    if (warp_id == producer_warp && elect_sync())
    {
        ...
        for (int k = 0; k < k_iters; ++k)
        {
            ...
            const int B_row = k * BK;
            const int B_col_scaled = block_n * BN / SWIZZLE_W;
            ...
            tma_4d_gmem2smem(
                B_smem, &B_tmap,
                0, B_row, B_col_scaled, batch, mbar);
        ...
        for (int k = 0; k < k_iters; ++k)
        {
            ...
            for (int k1 = 0; k1 < BK / SWIZZLE_W; ++k1)
            {
                for (int k2 = 0; k2 < SWIZZLE_W / MMA_K_STEP; ++k2)
                {
                    ...
                    // A is [BM, BK]. K is columns.
                    // Stepping k1 (chunk of 64 cols) skips BM * 64 elements.
                    // Stepping k2 (16 cols) skips 16 elements.
                    const int a_off = k1 * SWIZZLE_W * BM * (int)sizeof(__nv_bfloat16)
                                    + k2 * MMA_K_STEP  * (int)sizeof(__nv_bfloat16);
                    
                    const int b_off = k1 * SWIZZLE_W * (BN * 2)
                                    + k2 * MMA_K_STEP  * (BN * 2);

                    // A: K-major, B: MN-major. Both use standard swizzled descriptors with 0 offset.
...
static void launch_bgemm_sm103_bf16_nn(
    ...
    cudaStream_t stream)
{
    ...
    init_4d_tma_map_sm103_A(
        &A_tmap, A, K, BK,
        (uint64_t)M, (uint32_t)BM,
        (uint64_t)batchCount, (uint64_t)M * K,
        CUtensorMapSwizzle::CU_TENSOR_MAP_SWIZZLE_128B);

    // B: [batch, K, N] row-major (NN layout)
    init_4d_tma_map_sm103_B_NN(
        &B_tmap, B, N, BN, K, BK,
    ...
    auto kernel = bgemm_sm103_bf16_nn_kernel<BM, BN, BK, QUEUE_SIZE>;
...
extern "C" void mycublasBgemmSM103_bf16_nn_128x128x64(
    ...
    int batchCount)
{
    ...
    launch_bgemm_sm103_bf16_nn<128, 128, 64>(
        A, B, C, M, N, K, batchCount, alpha, beta, stream);
}

extern "C" void mycublasBgemmSM103_bf16_nn_strided(
    mycublasHandle_t handle,
    int M, int N, int K,
    float alpha,
